...
const en: Article[] = [
  ...
  },
  {
    ...
    date: '2026-08-18',
    title: 'How Much Does It Cost to Sell a Company? M&A Fees Explained',
    description:
    ...
    body: [
      ...
      { kind: 'h2', text: 'Why the cheapest adviser is rarely the cheapest outcome' },
      { kind: 'p', text: 'The difference between two fee quotes is usually a fraction of a percentage point of transaction value. The difference between a process with three credible bidders and a process with one is frequently ten to thirty per cent of the price — and the terms behind the headline number, where value quietly leaks through warranties, escrow and working capital adjustments. Fees are worth negotiating; they are not worth optimising at the expense of process quality.' },
      { kind: 'h2', text: 'How to keep costs under control' },
      { kind: 'p', text: 'Agree the whole fee structure at the outset, in writing: how transaction value is defined (cash, debt, earn-out and deferred consideration all matter), what expenses are recoverable and whether they are capped, and the length of the tail period after termination. Scope legal work in phases rather than open-ended. Above all, prepare early — the single biggest driver of transaction cost is not the fee schedule but delay, and delay comes from documents that were never in order.' },
      { kind: 'p', text: 'A credible adviser will explain their fees plainly and be comfortable with the questions above. If you are weighing up a sale, the first conversations — about timing, readiness and what the process would realistically cost — are free, and they are the ones that shape everything after.' },
    ],
  },
  {
    slug: 'restructuring-or-selling-a-company-under-pressure',
    date: '2026-08-24',
    title: 'Restructuring or Selling: Options When a Company Is Under Pressure',
    description:
  ...
  },
  {
    ...
    body: [
      ...
      { kind: 'p', text: 'Less than the headline suggests, because FDI and M&A measure different things. Foreign direct investment includes greenfield construction, recapitalisations and intercompany loans, far more than the purchase of existing companies. A fall speaks mainly to greenfield: less is being built new. For someone selling a running business that is close to neutral, and it can even help. When building from scratch is more expensive and slower, buying something that already works becomes more attractive by comparison — provided it survives scrutiny once a buyer opens the books.' },
...
const sr: Article[] = [
  ...
  },
  {
    ...
    body: [
      ...
      { kind: 'p', text: 'Prodaja nosi dve vrste troška: savetnika koji vodi proces i stručnjake koji transakciju čine pravno i finansijski čvrstom. Prvi je pretežno vezan za uspeh, pa se najveći deo plaća samo ako se posao zaključi. Drugi nastaje bez obzira na ishod — zbog čega je pravilan redosled aktivnosti važan.' },
      { kind: 'h2', text: 'Naknada savetnika: retainer i uspešnica' },
      { kind: 'p', text: 'Naknade M&A savetnika gotovo uvek se dele na retainer i naknadu za uspeh (success fee). Retainer je umeren mesečni ili inicijalni iznos koji finansira fazu pripreme — finansijsku analizu, procenu vrednosti, izradu prodajnih materijala i mapiranje kupaca — i predstavlja signal ozbiljnosti sa obe strane. U većini mandata se pri zaključenju transakcije uračunava u naknadu za uspeh.' },
      { kind: 'p', text: 'Naknada za uspeh je suština aranžmana: procenat od vrednosti transakcije, plativ tek kada se posao zatvori. Upravo ona usklađuje interese savetnika i prodavca — savetnik se plaća za zaključenu transakciju po najboljoj ostvarivoj ceni, a ne za aktivnost.' },
      { kind: 'h2', text: 'Koliko iznosi naknada za uspeh' },
      { kind: 'p', text: 'Naknada za uspeh kreće se obrnuto srazmerno veličini posla. Prodaja od 5 miliona evra traži gotovo isti obim rada kao i ona od 50 miliona, pa manje transakcije nose viši procenat. U srednjem segmentu tržišta uspešnica se najčešće kreće u rasponu nižih do srednjih jednocifrenih procenata, spuštajući se ka jedan do dva odsto kod većih transakcija. Mnogi mandati uvode i podsticajni sloj — višu stopu na vrednost ostvarenu iznad dogovorenog praga — koji savetnika nagrađuje upravo za rezultat iznad očekivanja, do koga je prodavcu i stalo.' },
      { kind: 'h2', text: 'Ostali troškovi transakcije' },
      { kind: 'ul', items: [
        'Advokati — pregovaranje ugovora o kupoprodaji udela, uređivanje korporativne dokumentacije i regulatorna odobrenja; po pravilu druga najveća stavka.',
        'Finansijski i poreski due diligence, ili vendor due diligence koji priprema sam prodavac kako bi predupredio nalaze kupca.',
        'Računovodstveni ili revizorski rad kojim se istorijski finansijski izveštaji dovode na nivo prihvatljiv kupcima.',
        'Praktični troškovi procesa — virtuelna soba podataka, prevodi dokumentacije, notarske i takse za registraciju.',
        'Takse za prijavu koncentracije kada transakcija zahteva odobrenje organa za zaštitu konkurencije.',
      ] },
      { kind: 'h2', text: 'Zašto najjeftiniji savetnik retko donosi najjeftiniji ishod' },
      { kind: 'p', text: 'Razlika između dve ponude za naknadu obično je delić procenta vrednosti transakcije. Razlika između procesa sa tri ozbiljna ponuđača i procesa sa jednim često je deset do trideset odsto cene — kao i uslovi iza same cifre, gde vrednost tiho curi kroz garancije, escrow i korekcije obrtnog kapitala. O naknadama vredi pregovarati; ne vredi ih optimizovati na štetu kvaliteta procesa.' },
      { kind: 'h2', text: 'Kako držati troškove pod kontrolom' },
      { kind: 'p', text: 'Dogovorite celu strukturu naknada na početku i u pisanoj formi: kako se definiše vrednost transakcije (gotovina, dug, earn-out i odloženo plaćanje — sve to je bitno), koji su troškovi nadoknadivi i da li su ograničeni, i koliko traje takozvani tail period nakon raskida mandata. Pravni rad ugovorite po fazama, a ne otvoreno. Pre svega, pripremajte se na vreme — najveći pojedinačni uzročnik troška nije cenovnik naknada, već kašnjenje, a kašnjenje nastaje iz dokumentacije koja nikada nije bila sređena.' },
      { kind: 'p', text: 'Ozbiljan savetnik svoje naknade objašnjava jednostavno i nema problem sa gore navedenim pitanjima. Ako razmišljate o prodaji, prvi razgovori — o trenutku, spremnosti i realnom trošku procesa — ne koštaju ništa, a oblikuju sve što sledi.' },
    ],
  },
  {
    slug: 'restructuring-or-selling-a-company-under-pressure',
    ...
    body: [
      ...
      { kind: 'p', text: 'Nijedan od ovih znakova sam po sebi nije poguban. Zajedno, međutim, govore da se kompanija približava tački u kojoj odluke umesto vlasnika počinje da donosi neko drugi — banka, poverilac ili sud.' },
      { kind: 'h2', text: 'Operativno i finansijsko restrukturiranje su dva različita problema' },
      { kind: 'p', text: 'Operativno restrukturiranje se bavi poslovanjem koje ne zarađuje dovoljno: troškovnom osnovom, portfoliom proizvoda i kupaca, neosnovnom imovinom i disciplinom u upravljanju obrtnim kapitalom. Finansijsko restrukturiranje se bavi poslovanjem koje funkcioniše, ali kapitalnom strukturom koja ne funkcioniše — previše duga, pogrešna ročnost, pogrešna valuta, krediti koji nikada nisu bili usklađeni sa gotovinom koju kompanija zaista stvara. Većina stvarnih situacija je kombinacija oba, a tretiranje problema bilansa kao operativnog problema, ili obrnuto, troši ono čega u ovakvim slučajevima ima najmanje: vreme.' },
      { kind: 'h2', text: 'Refinansiranje i dokapitalizacija' },
      { kind: 'p', text: 'Kada je samo poslovanje zdravo, prvi put je popravka bilansa — refinansiranje postojećeg duga na duže rokove, zamena kratkoročnih kredita, uvođenje novih kreditora ili mezanin kapitala, ili dokapitalizacija sredstvima vlasnika. Kreditori pregovaraju na osnovu verodostojnog plana i pouzdanih brojki, a kvalitet informacija koje se pred njih iznesu često je jednako važan kao i samo stanje. Obratiti im se sa dokumentovanim planom potpuno je drugačiji razgovor od obraćanja nakon što je rata već propuštena.' },
      { kind: 'h2', text: 'Ulazak partnera: delimična prodaja' },
      { kind: 'p', text: 'Prodaja manjinskog ili većinskog udela strateškom ili finansijskom investitoru donosi kapital, a često i sposobnosti — dubinu menadžmenta, pristup tržištima, snagu u nabavci. Vlasniku koji ne želi da izađe iz posla, delimična prodaja omogućava da rekapitalizuje kompaniju i zadrži učešće u onome što sledi. Ozbiljan novi suvlasnik menja i način na koji kompaniju vide kreditori, dobavljači i ključni kupci, što ume da vredi koliko i sam novac.' },
      { kind: 'h2', text: 'Prodaja kompanije, uključujući i prodaju pod pritiskom' },
      { kind: 'p', text: 'Prodaja je legitiman instrument restrukturiranja, a ne priznanje neuspeha. Ali ishod presudno zavisi od trenutka. Kompanija koja se prodaje dok još ima likvidnost, netaknut menadžerski tim i stvaran izbor kupaca vrednuje se kao poslovanje. Ista ta kompanija, prodata tri kvartala kasnije u procesu koji vode poverioci, vrednuje se kao skup imovine. Procesi pod pritiskom teku brže, privlače manje ponuđača i prebacuju više rizika i više odgovornosti po osnovu garancija na prodavca.' },
      { kind: 'h2', text: 'Formalna reorganizacija' },
      { kind: 'p', text: 'Kada su sporazumna rešenja iscrpljena, postoje i formalni postupci. U Srbiji unapred pripremljen plan reorganizacije (UPPR) omogućava dužniku da uslove restrukturiranja dogovori sa poveriocima i da ih sud potvrdi, čime plan obavezuje i poverioce iz iste klase koji su glasali protiv. Priprema se uz advokate i finansijskog savetnika i za pojedine kompanije predstavlja stvarno rešenje. Istovremeno je restriktivniji, javniji i skuplji od dogovora postignutog godinu dana ranije.' },
  ...
  },
  {
    ...
    body: [
      ...
      { kind: 'p', text: 'Zato što je alternativa način na koji se piše većina tržišnih komentara. Zbirni polugodišnji podaci za region u trenutku pisanja nisu bili objavljeni i mogli smo da izvedemo broj koji niko nikada ne bi proverio. Onog trenutka kada to jednom uradite, sve ostalo što objavite vredi manje. Vlasnik koji odlučuje da li da proda kompaniju građenu dvadeset godina zaslužuje da zna šta je od naših tvrdnji proverljivo, a šta je mišljenje. Tamo gde broja nema, tako i piše. Isti test bih primenio na svakog savetnika koji vam citira tržišni multiplikator — pitajte odakle mu.' },
